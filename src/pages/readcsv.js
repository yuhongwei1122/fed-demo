        ...
        // if(!runFlag){
            setTimeout(()=>{
                ['fed_fix','local_fix','preventive_fix','corrective_fix'].map((name,index)=>{
                    showLeftLearnCharts('oneModalChart'+index,'oneModalChart'+index,device,name,deviceData[device],500);
                });
                ['TsPreOut_abnormal','TpreIn_abnormal','Texh_abnormal','PAw_abnormal','TsEcoOut_abnormal','PAs0_abnormal','Ts0_abnormal'].map((name,index)=>{
                    showParamsCharts('oneModalParamsChart'+index,'oneModalParamsChart'+index,device,name,deviceData[device],1);
                })
            },100);
        // }
    };
        ...
        intervalFun = setInterval(() =>{
            ...
            if(upIndex == 4000/(COMMRUNSTEP)){
                ...
                const seriseData = ['corrective_profit','preventive_profit','local_profit','fed_profit'].map((name,index)=>{
                    ...
                    //         x: '93%',};
                    const temp = end < 500 ?{yAxis: deviceData[selectDevice][name][end-1],
                            ...
                            data : [
                                {
                                    ...
                                    name: '最后的值',
                                    // yAxis: deviceData[selectDevice][name][end-1],
                                    // x: upIndex*COMMRUNSTEP,
                                    ...temp,
                                    symbolSize:1,
                                    label:{
                                      normal:{
                ...
                profileChart.setOption({
                    ...
                    xAxis:{
                        data:deviceData[selectDevice]['time'].slice(start,end < 500 ? 500 :end),
                        // splitNumber:5,
                        axisLabel:{
                            interval:250
                        }
            ...
            >
                <div className='demo-left' style={{padding:'0px 16px'}}>
                    ...
                    <div style={{border:'1px solid #CED8E6',borderRadius:'4px',marginTop:20}}>
                        ...
                        <Row className='item-wrap' type="flex" justify="space-between" style={{margin:30}}>
                            <Col style={{paddingRight:40}} span={12} className='item-wrap-left border-ver'>
                                <Row style={{paddingBottom:15}}>
                                    ...
                                    <Col span={15} className='p-desc' style={{textAlign:'right',fontSize:'12px'}}>
                                        <span style={{transform:'scale(0.9)'}}>维护(次):{testCount(getOutputData(detailName,'fed_fix'),1)}</span>
                                        <Divider type="vertical" />
                                        <span style={{transform:'scale(0.9)'}}>故障(次):{testCount(getOutputData(detailName,'fed_fix'),3)}</span>
                                    </Col>
                                </Row>
                                <div className='small-charts'>
                                    <div className='small-charts-right hasbg' style={{paddingRight:10}}>
                                    ...
                                    <div className='small-charts-left' style={{color:'#8C96AA !important',transform:'scale(0.9)',width:'auto',textAlign:'right'}}>{4000}(d)</div>
                                </div>
                            </Col>
                            <Col style={{paddingLeft:40}} span={12} className='item-wrap-right' >
                                <Row style={{paddingBottom:15}}>
                                    ...
                                    <Col span={15} className='p-desc' style={{textAlign:'right',fontSize:'12px'}}>
                                        <span style={{transform:'scale(0.9)'}}>维护(次):{testCount(getOutputData(detailName,'local_fix'),1)}</span>
                                        <Divider type="vertical" />
                                        <span style={{transform:'scale(0.9)'}}>故障(次):{testCount(getOutputData(detailName,'local_fix'),3)}</span>
                                    </Col>
                                </Row>
                                <div className='small-charts'>
                                    <div className='small-charts-right hasbg' style={{paddingRight:10}}>
                                    ...
                                    <div className='small-charts-left' style={{color:'#8C96AA !important',transform:'scale(0.9)',width:'auto'}}>{4000}(d)</div>
                                </div>
                            </Col>
                        </Row>
                        <Row className='item-wrap' type="flex" justify="space-between" style={{margin:30}}>
                            <Col style={{paddingRight:40}} span={12} className='item-wrap-left border-ver'>
                                <Row style={{paddingBottom:15}}>
                                    ...
                                    <Col span={15} className='p-desc' style={{textAlign:'right',fontSize:'12px'}}>
                                        <span style={{transform:'scale(0.9)'}}>维护(次):{testCount(getOutputData(detailName,'preventive_fix'),1)}</span>
                                        <Divider type="vertical" />
                                        <span style={{transform:'scale(0.9)'}}>故障(次):{testCount(getOutputData(detailName,'preventive_fix'),3)}</span>
                                    </Col>
                                </Row>
                                <div className='small-charts'>
                                    <div className='small-charts-right hasbg' style={{paddingRight:10}}>
                            ...
                            <Col style={{paddingLeft:40}} span={12} className='item-wrap-right' >
                                <Row style={{paddingBottom:15}}>
                                    ...
                                    <Col span={15} className='p-desc' style={{textAlign:'right',fontSize:'12px'}}>
                                        <span style={{transform:'scale(0.9)'}}>维护(次):{testCount(getOutputData(detailName,'corrective_fix'),1)}</span>
                                        <Divider type="vertical" />
                                        <span style={{transform:'scale(0.9)'}}>故障(次):{testCount(getOutputData(detailName,'corrective_fix'),3)}</span>
                                    </Col>
                                </Row>
                                <div className='small-charts'>
                                    <div className='small-charts-right hasbg' style={{paddingRight:10}}>
                        ...
                        <Row className='item-bottom'>
                            {
                                ['TsPreOut_abnormal','TpreIn_abnormal','Texh_abnormal','PAw_abnormal','TsEcoOut_abnormal','PAs0_abnormal','Ts0_abnormal'].map((paramKey,index)=>{
                ...
                <div style={{textAlign:'center',margin:'30px 16px 16px'}}>
                    <Button style={{borderRadius:'18px',fontWeight:'bold'}} onClick={()=>setDetailFlag(false)} className='reset-btn'>关闭</Button>
                </div>
            </Modal>
        </div>
    </div>
...
const selectBarOption = (deviceData,modal,end) => { 
    ...
    return {
        xAxis: {
            type: 'category',
            data: deviceData['time'].slice(-end),
            show:false
        },
        yAxis: {
            type: 'value',
